...
If no [compositor] table exists, Knave projects root-level modkey,
...
Source data must not be silently deleted.

The tracked root `config.example.toml` is retained only as a root-level migration
